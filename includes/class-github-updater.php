<?php
/**
 * GitHub Plugin Updater for Distribution - Listmonk
 *
...
class Dist_Listmonk_GitHub_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-dist-listmonk')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'dist_listmonk_github_update_' . md5($repo);
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) array(
            ...
            'sections' => array(
                'description' => 'Listmonk newsletter integration for Parish Distribution.',
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?dist_listmonk_check_update=1'), 'dist_listmonk_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['dist_listmonk_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'dist_listmonk_check_update')) {
            return;
        }
